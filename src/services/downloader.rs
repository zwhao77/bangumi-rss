//! Aria2 downloader — JSON-RPC client behind the `TorrentDownloader` trait.
//!
//! Uses an **infohash → gid cache** with per-call validation:
//!   1. Cache hit → `tellStatus(gid, ["infoHash"])` to verify
//!   2. Valid → use gid; stale → rebuild from `tellActive` + `tellStopped`
//!   3. Retry lookup after rebuild.

use std::collections::HashMap;
use std::sync::Mutex;

use crate::traits::{CompletedDownload, TorrentDownloader, TorrentFile};
use crate::types::DownloadSnapshot;

/// Concrete downloader backed by aria2's JSON-RPC API.
pub struct Aria2Downloader {
    rpc_url: String,
    /// infohash → gid.  Populated on add_uri / poll_completed / rebuild.
    cache: Mutex<HashMap<String, String>>,
}

impl Aria2Downloader {
    pub fn from_env() -> Self {
        Self {
            rpc_url: std::env::var("ARIA2_RPC_URL")
                .unwrap_or_else(|_| "http://localhost:6800/jsonrpc".into()),
            cache: Mutex::new(HashMap::new()),
        }
    }

    // ── Low-level JSON-RPC ──

    fn rpc(&self, method: &str, params: &[serde_json::Value]) -> Option<serde_json::Value> {
        let payload = serde_json::json!({
            "jsonrpc": "2.0",
            "id": "1",
            "method": format!("aria2.{method}"),
            "params": params,
        });
        let resp = ureq::post(&self.rpc_url).send_json(payload).ok()?;
        let body: serde_json::Value = resp.into_json().ok()?;
        if body.get("error").is_some() {
            return None;
        }
        Some(body["result"].clone())
    }

    // ── Cache management ──

    /// Validate that a cached gid still points to the expected infohash.
    fn validate_gid(&self, gid: &str, expected_infohash: &str) -> bool {
        match self.rpc(
            "tellStatus",
            &[serde_json::json!(gid), serde_json::json!(["infoHash"])],
        ) {
            Some(status) => status["infoHash"].as_str() == Some(expected_infohash),
            None => false,
        }
    }

    /// Rebuild the infohash → gid cache from all active / stopped tasks.
    fn rebuild_cache(&self) {
        let mut cache = self.cache.lock().unwrap();
        cache.clear();

        let tasks = self
            .rpc("tellActive", &[serde_json::json!(["gid", "infoHash"])])
            .and_then(|r| r.as_array().cloned())
            .unwrap_or_default()
            .into_iter()
            .chain(
                self.rpc(
                    "tellStopped",
                    &[
                        serde_json::json!([-1, 1000]),
                        serde_json::json!(["gid", "infoHash"]),
                    ],
                )
                .and_then(|r| r.as_array().cloned())
                .unwrap_or_default(),
            );

        for t in tasks {
            if let (Some(ih), Some(gid)) = (t["infoHash"].as_str(), t["gid"].as_str()) {
                cache.insert(ih.to_string(), gid.to_string());
            }
        }
        println!("[aria2] cache rebuilt: {} entries", cache.len());
    }

    /// Resolve infohash → gid, validating and rebuilding on miss.
    fn with_gid(&self, infohash: &str) -> anyhow::Result<String> {
        // 1. Try cache + validate
        {
            let cache = self.cache.lock().unwrap();
            if let Some(gid) = cache.get(infohash) {
                if self.validate_gid(gid, infohash) {
                    return Ok(gid.clone());
                }
            }
        }

        // 2. Cache miss or stale → full rebuild
        self.rebuild_cache();

        // 3. Retry
        let cache = self.cache.lock().unwrap();
        cache
            .get(infohash)
            .cloned()
            .ok_or_else(|| anyhow::anyhow!("task not found in aria2: {infohash}"))
    }
}

impl TorrentDownloader for Aria2Downloader {
    fn add_uri(&self, uri: &str, dir: &str) -> anyhow::Result<String> {
        let result = self.rpc(
            "addUri",
            &[serde_json::json!([uri]), serde_json::json!({ "dir": dir })],
        );
        let gid = result
            .and_then(|r| r.as_str().map(String::from))
            .ok_or_else(|| anyhow::anyhow!("aria2.addUri returned no GID"))?;

        let infohash = (0..30)
            .find_map(|i| {
                if i > 0 {
                    std::thread::sleep(std::time::Duration::from_secs(1));
                }
                self.rpc(
                    "tellStatus",
                    &[serde_json::json!(gid), serde_json::json!(["infoHash"])],
                )
                .and_then(|s| s["infoHash"].as_str().map(String::from))
                .filter(|s| !s.is_empty())
            })
            .ok_or_else(|| {
                anyhow::anyhow!(
                    "aria2.addUri succeeded (gid={gid}) but tellStatus returned no infoHash after 30 retries"
                )
            })?;

        if !infohash.is_empty() {
            self.cache.lock().unwrap().insert(infohash.clone(), gid);
        }
        Ok(infohash)
    }

    fn add_torrent_bytes(&self, data: &[u8], dir: &str) -> anyhow::Result<String> {
        let b64 = {
            use base64::Engine;
            base64::engine::general_purpose::STANDARD.encode(data)
        };
        let result = self.rpc(
            "addTorrent",
            &[
                serde_json::json!(b64),
                serde_json::json!([]),
                serde_json::json!({ "dir": dir }),
            ],
        );
        let gid = result
            .and_then(|r| r.as_str().map(String::from))
            .ok_or_else(|| anyhow::anyhow!("aria2.addTorrent returned no GID"))?;

        let infohash = self
            .rpc(
                "tellStatus",
                &[serde_json::json!(gid), serde_json::json!(["infoHash"])],
            )
            .and_then(|s| s["infoHash"].as_str().map(String::from))
            .unwrap_or_default();

        if !infohash.is_empty() {
            self.cache.lock().unwrap().insert(infohash.clone(), gid);
        }
        Ok(infohash)
    }

    fn list_files(&self, infohash: &str) -> anyhow::Result<Vec<TorrentFile>> {
        let gid = self.with_gid(infohash)?;
        let result = self.rpc("getFiles", &[serde_json::json!(gid)]);
        let arr = result
            .and_then(|r| r.as_array().cloned())
            .unwrap_or_default();
        Ok(arr
            .iter()
            .filter_map(|f| {
                let path = f["path"].as_str()?.to_string();
                let name = std::path::Path::new(&path)
                    .file_name()?
                    .to_string_lossy()
                    .to_string();
                let size = f["length"]
                    .as_str()
                    .and_then(|s| s.parse().ok())
                    .unwrap_or(0);
                Some(TorrentFile { name, size })
            })
            .collect())
    }

    fn rename_file(&self, infohash: &str, _old_path: &str, new_name: &str) -> anyhow::Result<bool> {
        let gid = self.with_gid(infohash)?;
        let result = self.rpc(
            "changeOption",
            &[
                serde_json::json!(gid),
                serde_json::json!({ "out": new_name }),
            ],
        );
        Ok(result.is_some())
    }

    fn poll_completed(&self) -> anyhow::Result<Vec<CompletedDownload>> {
        // Query stopped tasks with status "complete".
        let stopped = self.rpc(
            "tellStopped",
            &[
                serde_json::json!([-1, 1000]),
                serde_json::json!(["gid", "infoHash", "status"]),
            ],
        );
        // Also query active tasks — those at 100% are seeding (= done).
        let active = self.rpc(
            "tellActive",
            &[serde_json::json!([
                "gid",
                "infoHash",
                "status",
                "totalLength",
                "completedLength"
            ])],
        );

        let mut completed = Vec::new();

        // Stopped + complete.
        for t in stopped
            .as_ref()
            .and_then(|r| r.as_array())
            .cloned()
            .unwrap_or_default()
        {
            if t["status"].as_str() != Some("complete") {
                continue;
            }
            if let (Some(ih), Some(gid)) = (t["infoHash"].as_str(), t["gid"].as_str()) {
                self.cache
                    .lock()
                    .unwrap()
                    .insert(ih.to_string(), gid.to_string());
                completed.push(CompletedDownload {
                    infohash: ih.to_string(),
                });
            }
        }

        // Active + 100% = seeding → treat as completed.
        for t in active
            .as_ref()
            .and_then(|r| r.as_array())
            .cloned()
            .unwrap_or_default()
        {
            let total: u64 = t["totalLength"]
                .as_str()
                .and_then(|s| s.parse().ok())
                .unwrap_or(0);
            let done: u64 = t["completedLength"]
                .as_str()
                .and_then(|s| s.parse().ok())
                .unwrap_or(0);
            if total == 0 || done < total {
                continue;
            }
            if let (Some(ih), Some(gid)) = (t["infoHash"].as_str(), t["gid"].as_str()) {
                self.cache
                    .lock()
                    .unwrap()
                    .insert(ih.to_string(), gid.to_string());
                completed.push(CompletedDownload {
                    infohash: ih.to_string(),
                });
            }
        }

        Ok(completed)
    }

    fn poll_failed(&self) -> anyhow::Result<Vec<CompletedDownload>> {
        let result = self.rpc(
            "tellStopped",
            &[
                serde_json::json!([-1, 1000]),
                serde_json::json!(["gid", "infoHash", "status"]),
            ],
        );
        let arr = result
            .and_then(|r| r.as_array().cloned())
            .unwrap_or_default();

        let mut failed = Vec::new();
        for t in &arr {
            if t["status"].as_str() != Some("error") {
                continue;
            }
            if let (Some(ih), Some(gid)) = (t["infoHash"].as_str(), t["gid"].as_str()) {
                self.cache
                    .lock()
                    .unwrap()
                    .insert(ih.to_string(), gid.to_string());
                failed.push(CompletedDownload {
                    infohash: ih.to_string(),
                });
            }
        }
        Ok(failed)
    }

    fn query_all(&self) -> anyhow::Result<Vec<DownloadSnapshot>> {
        let mut snapshots = Vec::new();

        let tasks = self
            .rpc(
                "tellActive",
                &[serde_json::json!([
                    "gid",
                    "infoHash",
                    "status",
                    "totalLength",
                    "completedLength",
                    "downloadSpeed",
                    "bittorrent"
                ])],
            )
            .and_then(|r| r.as_array().cloned())
            .unwrap_or_default()
            .into_iter()
            .chain(
                self.rpc(
                    "tellStopped",
                    &[
                        serde_json::json!([-1, 1000]),
                        serde_json::json!([
                            "gid",
                            "infoHash",
                            "status",
                            "totalLength",
                            "completedLength",
                            "downloadSpeed",
                            "bittorrent"
                        ]),
                    ],
                )
                .and_then(|r| r.as_array().cloned())
                .unwrap_or_default(),
            );

        for t in tasks {
            let infohash = t["infoHash"].as_str().unwrap_or("").to_string();
            if infohash.is_empty() {
                continue;
            }
            let gid = t["gid"].as_str().unwrap_or("");
            let total: u64 = t["totalLength"]
                .as_str()
                .and_then(|s| s.parse().ok())
                .unwrap_or(0);
            let done: u64 = t["completedLength"]
                .as_str()
                .and_then(|s| s.parse().ok())
                .unwrap_or(0);
            let progress = if total > 0 {
                done as f32 / total as f32
            } else {
                0.0
            };
            let state = match t["status"].as_str().unwrap_or("") {
                "complete" => crate::types::DownloadState::Completed,
                "active" if done >= total && total > 0 => crate::types::DownloadState::Seeding,
                "active" => crate::types::DownloadState::Downloading,
                "waiting" => crate::types::DownloadState::Waiting,
                "paused" => crate::types::DownloadState::Paused,
                "error" => crate::types::DownloadState::Failed,
                _ => continue,
            };
            let speed: u64 = t["downloadSpeed"]
                .as_str()
                .and_then(|s| s.parse().ok())
                .unwrap_or(0);
            let name = t["bittorrent"]["info"]["name"]
                .as_str()
                .unwrap_or(gid)
                .to_string();

            // Keep cache warm.
            if !gid.is_empty() {
                self.cache
                    .lock()
                    .unwrap()
                    .insert(infohash.clone(), gid.to_string());
            }

            snapshots.push(DownloadSnapshot {
                infohash,
                state,
                progress,
                speed,
                size: total,
                name,
            });
        }
        Ok(snapshots)
    }
}

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn cache_new_is_empty() {
        let dl = Aria2Downloader {
            rpc_url: String::new(),
            cache: Mutex::new(HashMap::new()),
        };
        assert!(dl.cache.lock().unwrap().is_empty());
    }

    #[test]
    fn with_gid_miss_returns_error() {
        let dl = Aria2Downloader {
            rpc_url: String::new(),
            cache: Mutex::new(HashMap::new()),
        };
        let result = dl.with_gid("deadbeef");
        assert!(result.is_err());
    }

    #[test]
    fn validate_gid_no_server_returns_false() {
        let dl = Aria2Downloader {
            rpc_url: String::new(),
            cache: Mutex::new(HashMap::new()),
        };
        assert!(!dl.validate_gid("gid", "infohash"));
    }
}
