...
- **TODO**: Mock downloader can't exercise the completion path end-to-end — `MockFileSystem` is a stub (`exists` only checks an empty `existing` set, `move_file` only records moves) and `MockDownloader`/`MockFileSystem` are separate instances, so `list_files` results never exist in the mock FS (`fallback: file not found`, tracker stuck at `Downloading`). Before testing the multi-episode TODO without a real downloader, share one `MockFileSystem` between downloader and `FileOps`, register placeholder files on `add_*`, and make `move_file` actually move entries (keeping `moves` for existing assertions).
