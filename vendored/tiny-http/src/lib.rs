...
pub struct ServerConfig {
    ...
    /// Maximum number of concurrent connections. `None` = unlimited (default).
    pub max_connections: Option<usize>,

    /// Maximum pending tasks in queue. `None` = unlimited (default).
    /// Excess connections are dropped immediately when the queue is full.
...
impl Server {
    ...
    pub fn new(config: ServerConfig) -> Result<Server, Box<dyn Error + Send + Sync + 'static>> {
        ...
        Self::from_listener(listener, config.ssl, config.max_connections, config.max_queue)
    }
    ...
    pub fn from_listener<L: Into<Listener>>(
        ...
        thread::spawn(move || {
            let tasks_pool = util::TaskPool::new_with_limits(max_connections, max_queue);
